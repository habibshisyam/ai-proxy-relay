...
export default async function handler(request) {
  ...
  const healthHosts = (process.env.HEALTH_HOSTS || "httpbin.org,api.httpbin.org,www.google.com")
    .split(",")
  ...
  try {
    ...
    return json(
      {
        error: aborted ? "Upstream timeout after 60s" : error.message,
        target: targetUrl,
      },
      aborted ? 504 : 502
    );
